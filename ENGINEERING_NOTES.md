...
Beyond the unit tests, a resilience suite (`webhook-resilience.test.ts`) proves the claim protocol against a **real Postgres**, not a mock — because the guarantee being tested (the unique-constraint claim and the conditional-`updateMany` CAS) lives in the database, a mock would only confirm the mock's own behavior. It exercises the exact query sequence the handler and worker use and asserts four invariants: only one concurrent claim wins, only one CAS reclaim mutates a contended row, an event that fails past `MAX_ATTEMPTS` lands in `dead_letter` and stops being a worker candidate, and a stale `processing` row becomes reclaimable. The suite skips cleanly when no `DATABASE_URL` is set (so the fast unit run needs no database) and runs against a Postgres service container in CI on every push (`.github/workflows/test.yml`), so the concurrency guarantee is verified automatically rather than asserted.
...
The concurrency claims above aren't just asserted — the resilience suite (§2, *Tests*) proves the claim protocol against a real Postgres in CI: one winner per concurrent claim, one winner per contended reclaim, terminal dead-letter, and stale-row recovery.
...
- **Extend test coverage to the full handler path.** The claim protocol is already proven against a real Postgres in CI (§2, *Tests*) — concurrent claims, contended reclaims, dead-letter, and stale recovery. The remaining gap is an end-to-end test that drives a request *through the route handler itself* (signature verification → claim → `processEvent` → response code) rather than exercising the claim queries in isolation, plus the same for the worker's scan-and-claim loop. That would close the distance between "the claim protocol is correct" and "the handler wires it correctly."
